...
import { getProviders, signIn as SigneIntoProvider } from "next-auth/react";
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen py-2 -mt-20 px-14 text-center">
        ...
        <p className="font-xs italic">This is not a real app it's built for educational purposes only</p>
        <div className="mt-40">
          {Object.values(providers).map((provider) => (
            <div key={provider.name}>
              <button
                className="p-3 bg-blue-500 rounded-lg text-white"
                onClick={() => SigneIntoProvider(provider.id, {callbackUrl:"/"})}
              >
